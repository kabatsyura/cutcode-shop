...
@section('content')
    <x-forms.auth-forms title='Вход в аккаунт' action="{{ route('login') }}" method="POST">
        @csrf
        <x-forms.text-input name="email" value="{{ old('email') }}" type="email" placeholder="E-mail" required="true"
        ...
        <x-forms.text-input name="password" type="password" placeholder="Пароль" required="true" :isError="$errors->has('email')" />
